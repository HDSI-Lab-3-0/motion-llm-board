...
from openrouter.wordbanks_spooky_no_maybe import (
    YES_NO,
    KEYWORDS,
    WORD_BANKS,
)
...
def pick_one_word(question: str) -> str:
    ...
    # Ask the LLM to choose from the bank, not invent something too long/weird.
...
- No explanations.
- Never return MAYBE.
- Prefer the creepiest appropriate word for the user's question.

Question: {question.strip()}
""".strip()

    out = openrouter_completion(prompt, max_tokens=4, temperature=0.4)
    out_up = sanitize_word(out)

    if out_up in [sanitize_word(w) for w in bank]:
        return out_up

    # Safety fallback from the correct spooky category.
